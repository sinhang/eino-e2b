...
func main() {
	...
	fsMiddleware, err := filesystem.New(ctx, &filesystem.MiddlewareConfig{
		Backend: backend,
		Shell:   backend, // enables the execute tool for shell commands
	})
	if err != nil {
...
func printSetupSummary(sandboxID string) {
	...
	sb.WriteString("  Middleware   : filesystem.New (ChatModelAgentMiddleware)\n")
